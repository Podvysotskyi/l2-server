...
internal static partial class CredentialRequestValidator
{
    ...
    public static Dictionary<string, string[]> Validate(RegistrationRequest request)
    {
        ...
        if (!errors.ContainsKey("password") &&
            !(request.Password ?? string.Empty).EnumerateRunes().Any(rune =>
                Rune.IsDigit(rune) || (!Rune.IsLetterOrDigit(rune) && !Rune.IsWhiteSpace(rune))))
        {
